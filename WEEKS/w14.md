...
- 구조:
  ...
  ```
- do - done 구간을 바다라고 부른다.
- 바다 구간은 var 개수만큼 반복이 되고, 마지막 var을 타고 종료가 된다.

Q. file1, file2, file3를 돌면서 파일 출력하기 
#!/bin/bash

for var in file1 file2 file3; do
    if [ -f "$var" ]; then  # 파일 존재 여부 확인
        echo "파일 내용: $var"
        cat "$var"  # 파일 내용 출력
        echo ""  # 빈 줄 추가 (가독성 향상)
    else
        echo "파일을 찾을 수 없습니다: $var"
    fi
done
...
for file in *         #여기서 *은 모든 디렉토리를 지정함.
